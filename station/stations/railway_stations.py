...
def make_station(station_tiles, entries, layout, base_id, notes=None):
    station_tiles.append(
        AStation(
            id=base_id,
            translation_name="RAIL_NAME",
            layouts=[layout, layout.M],
    ...
    if layout.traversable:
        wp_l = replace(layout, notes=["waypoint"])
        station_tiles.append(
            AStation(
                id=0x80 + base_id,
...
def register():
    ...
    make_station(station_tiles, entries, replace(track_layout, category=ENS_CLASS), 0x00)
    make_station(station_tiles, entries, replace(grassland_tile, category=ENS_CLASS), 0x10)
    make_station(station_tiles, entries, replace(building_ground_tile, category=ENS_CLASS), 0x11)
